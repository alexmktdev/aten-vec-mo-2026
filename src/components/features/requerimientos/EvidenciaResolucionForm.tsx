...
import { useSetEvidenciaResolucion } from "@/hooks/useRequerimientos";
...
export function EvidenciaResolucionForm({ requerimientoId, evidenciaExistente }: Props) {
  ...
  const handleSubmitDocumento = async () => {
    ...
    try {
      ...
      setSuccess("Evidencia de resolución adjuntada exitosamente");
      setPdfFile(null);
  ...
  const handleSubmitLink = async () => {
    ...
    try {
      ...
      setSuccess("Enlace de evidencia guardado exitosamente");
      setLinkUrl("");
      setTimeout(() => setSuccess(""), 4000);
    } catch (err) {
